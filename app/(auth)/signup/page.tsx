...
export default function SignupPage() {
    ...
    return (
        ...
        @media (max-width: 480px) {
          .signup-container { padding: 12px !important; }
          .signup-card { border-radius: 18px !important; }
          .signup-header { padding: 20px 20px 16px !important; }
          .signup-body { padding: 16px 20px 24px !important; }
      ...
      `}</style>
            ...
            <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#08080f', position: 'relative', overflow: 'hidden', fontFamily: 'Geist, sans-serif' }}>
                ...
                <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} className="signup-container" style={{ width: '100%', maxWidth: 416, position: 'relative', zIndex: 10, padding: 16 }}>
                    <div className="signup-card" style={{ background: 'rgba(12,10,22,0.9)', backdropFilter: 'blur(32px)', borderRadius: 23, border: '1px solid rgba(139,92,246,0.18)', overflow: 'hidden' }}>
                        <div className="signup-header" style={{ background: 'linear-gradient(135deg, rgba(109,40,217,0.15) 0%, rgba(88,28,235,0.07) 100%)', borderBottom: '1px solid rgba(139,92,246,0.1)', padding: '26px 32px 22px' }}>
                        ...
                        <div className="signup-body" style={{ padding: '22px 32px 30px' }}>
                            <AnimatePresence mode="wait">
                                {done ? (
                                    <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: '24px 0' }}>
                                        ...
                                        <p style={{ fontSize: 13, color: 'rgba(160,150,200,0.7)', marginBottom: 20 }}>Verification link sent to <strong style={{ color: '#c4a7ff' }}>{email}</strong></p>
                                        <Link href="/login" className="submit-btn-s" style={{ textDecoration: 'none' }}>Back to Sign In</Link>
                                    </motion.div>
                                ) : (
                                    <motion.div key="form">
                                        <button onClick={handleGoogle} disabled={googleLoading} className="social-btn-s" style={{ marginBottom: 18 }}>
                                            {googleLoading ? <Spinner /> : <GoogleIcon />} Sign up with Google
                                        </button>
                                        ...
                                        <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                                ...
                                                <div key={key}>
                                                    ...
                                                    <div style={{ position: 'relative' }}>
                                                        <div style={{ position: 'absolute', left: 13, top: '50%', transform: 'translateY(-50%)', zIndex: 10, color: focusedField === key ? 'rgba(167,139,250,0.7)' : 'rgba(140,130,180,0.4)' }}>
                                                            {icon === 'email' ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg> : <svg width="13" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>}
                                                        </div>
                                                        <input type={type} placeholder={placeholder} value={val} onChange={e => set(e.target.value)} onFocus={() => setFocusedField(key)} onBlur={() => setFocusedField(null)} className="signup-input" style={{ padding: '0 48px 0 38px' }} required />
                                                        {eye && <button type="button" onClick={() => setShowPass(!showPass)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', zIndex: 10, color: 'rgba(140,130,180,0.45)' }}><EyeIcon open={showPass} /></button>}
                                                    </div>
                                                </div>
                                            ))}
                                            ...
                                            <button type="submit" disabled={loading} className="submit-btn-s">Create Account</button>
                                        </form>
